...
def train(config):
    ...
    weatherData = 'data/ADDO ELEPHANT PARK.csv'
    ts = utils.create_dataset(weatherData)

    # Process data for T-GCN model
    data = ts[['Pressure', 'WindDir', 'WindSpeed', 'Humidity', 'Rain', 'Temperature']]
    adj = 'adj_mx'
    adjacency_matrix = pd.read_csv('data/adj_mx.csv')
    ...
    adjacency_matrix = np.mat(adjacency_matrix, dtype=np.float32)
    ...
    trainX, trainY, testX, testY = processing_data(data1, adjacency_matrix, time_len, train_rate, seq_len, pre_len, model_name, scheme)
    ...
    inputs = tf.keras.Input(shape=[seq_len, data1.shape[1]], dtype=tf.float32)
    labels = tf.keras.Input(shape=[pre_len, data1.shape[1]], dtype=tf.float32)
    ...
    label = tf.compat.v1.reshape(labels, [-1, data1.shape[1]])
    ...
    print("Training loop of the model.")
    ...
    for epoch in range(training_epoch):
        ...
        test_label = np.reshape(testY, [-1, data1.shape[1]])
        ...
        test_smape.append(smape * max_value)
        test_acc.append(acc)
    ...
    # Store the evaluation results or perform further analysis as needed
